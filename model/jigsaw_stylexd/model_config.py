from easydict import EasyDict as edict

__C = edict()
model_cfg = __C

__C.STYLEXD = edict()

__C.STYLEXD.ROT_TYPE = 'rmat'

__C.STYLEXD.USE_POINT_FEATURE = True
__C.STYLEXD.USE_LOCAL_POINT_FEATURE = True
__C.STYLEXD.USE_GLOBAL_POINT_FEATURE = False
__C.STYLEXD.USE_UV_FEATURE = False
__C.STYLEXD.USE_LOCAL_UV_FEATURE = False
__C.STYLEXD.USE_GLOBAL_UV_FEATURE = False
__C.STYLEXD.ENCODE_ALL_ONCE = False  # 不论使用点云、UV、或是法线特征，都使用同一个encoder提取特征
__C.STYLEXD.PC_FEAT_DIM = 128
__C.STYLEXD.UV_FEAT_DIM = 128
__C.STYLEXD.AFF_FEAT_DIM = 512
__C.STYLEXD.AFFINITY = 'aff_dual'
__C.STYLEXD.ENCODER = 'pointnet2_pt.msg.dynamic'
# __C.STYLEXD.TEST_S_MASK = True
__C.STYLEXD.PC_NUM_CLS = 2
__C.STYLEXD.PC_CLS_THRESHOLD = 0.8

# 是否分阶段训练模型
__C.STYLEXD.IS_TRAIN_IN_STAGE = False
__C.STYLEXD.TRAIN_IN_STAGE = edict()
__C.STYLEXD.TRAIN_IN_STAGE.W_CLS = 0.02
__C.STYLEXD.TRAIN_IN_STAGE.VAL_ACC = 95.

__C.STYLEXD.SINKHORN_MAXITER = 20
__C.STYLEXD.SINKHORN_TAU = 0.05

__C.STYLEXD.TF_LAYER_NUM = 1
__C.STYLEXD.TF_NUM_HEADS = 8
__C.STYLEXD.TF_NUM_SAMPLE = 16
__C.STYLEXD.USE_TF_BLOCK = False  # 是否使用 PointTransformer Block 来将每个 PointTransformer 进行分别封装
__C.STYLEXD.TF = edict()
__C.STYLEXD.TF.NORM = "batch"  # choice: "batch" , "instance"  TODO 应用到代码中

__C.STYLEXD.FEATURE_CONV = edict()
__C.STYLEXD.FEATURE_CONV.USE_FEATURE_CONV = False
__C.STYLEXD.FEATURE_CONV.TYPE = "default"               # 默认版本，与HOLA中保持一致
__C.STYLEXD.FEATURE_CONV.LAYER_NUM = 1
__C.STYLEXD.FEATURE_CONV.KERNEL_SIZE = 3
__C.STYLEXD.FEATURE_CONV.DILATION = 1

__C.STYLEXD.FEATURE_CONV_2 = edict()
__C.STYLEXD.FEATURE_CONV_2.USE_FEATURE_CONV = False
__C.STYLEXD.FEATURE_CONV_2.TYPE = "default"              # 可自定义调整模型层数
__C.STYLEXD.FEATURE_CONV_2.LAYER_NUM = 1
__C.STYLEXD.FEATURE_CONV_2.KERNEL_SIZE = 3
__C.STYLEXD.FEATURE_CONV_2.DILATION = 1

__C.STYLEXD.LOSS = edict()
"""
value of MAT_LOSS_TYPE should be selected in ["local", "global"]
local : only calculate mat loss in stitch loss, witch may cause cls loss raise
global : calculate mat loss in all points
"""
__C.STYLEXD.LOSS.MAT_LOSS_TYPE = "local"
__C.STYLEXD.LOSS.MAT_LOSS_SYM = True
__C.STYLEXD.LOSS.w_cls_loss = 0.5
__C.STYLEXD.LOSS.w_mat_loss = 0.0


# === 如果是分别训练的版本，则有些东西分开 ===
__C.STYLEXD.POINTCLASSIFIER = edict()
__C.STYLEXD.POINTCLASSIFIER.TF_LAYER_NUM = 1
__C.STYLEXD.POINTCLASSIFIER.TF_NUM_HEADS = 8
__C.STYLEXD.POINTCLASSIFIER.TF_NUM_SAMPLE = 16
__C.STYLEXD.POINTCLASSIFIER.USE_TF_BLOCK = False
__C.STYLEXD.POINTCLASSIFIER.NORM = "batch"  # choice: "batch" , "instance"

__C.STYLEXD.STITCHPREDICTOR = edict()
__C.STYLEXD.STITCHPREDICTOR.TF_LAYER_NUM = 1
__C.STYLEXD.STITCHPREDICTOR.TF_NUM_HEADS = 8
__C.STYLEXD.STITCHPREDICTOR.TF_NUM_SAMPLE = 16
__C.STYLEXD.STITCHPREDICTOR.USE_TF_BLOCK = False
__C.STYLEXD.STITCHPREDICTOR.NORM = "batch"  # choice: "batch" , "instance"


def get_model_cfg():
    return model_cfg.STYLEXD
