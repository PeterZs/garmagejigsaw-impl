...
def get_optimize_ST(pcs_list):
    ...
    for it in range(max(max_iter_s, max_iter_t)):
        ...
        # Apply positional transformation and compute the loss after the position change
        pcs_updated = []
        for idx, pcs in enumerate(pcs_list):
            ...
            pcs_updated.append(pcs_u)  # update pointcloud
        ...
        panel_loss /= len(pcs_updated)

        optimizer_st.zero_grad()
        loss_total = w_loss * panel_loss
        loss_total.backward()
        optimizer_st.step()
        # print(f'Iter {it} loss_total:{loss_total:.5f}')  # print loss for test
...
def get_pcs_selected(pcs_list):
    """
    For each panel, filter out points that are too far from other panels.
    :param pcs_list:
    :return:
    """
    result_pcs_list=[]
