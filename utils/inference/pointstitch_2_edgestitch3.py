"""
从点点缝合关系获取边边缝合关系
第三版，为了进行重构，让逻辑更加清晰，从而进行优化


第二版的逻辑：
    获取N个点点缝合组（Nx2个缝合点组）：通过一个很逻辑很混乱的算法
        缺点：
            错误的点点缝合影响很明显，会导致一个完整的点点缝合组由于其中的一个错误的点点缝合被拆分
            过多的拆分会导致后面的步骤中错误累积
    筛掉太短的点点缝合组
    对每个缝合点组，计算其中的时针方向
        缺点：
            “获取N个点点缝合组（Nx2个缝合点组）”步骤中过分的拆分导致这一步有些缝合点组的方向会预测错
    对一个缝合点组上的点，按照id进行排序
        缺点：
            此时，点点缝合组还没进行进一步拆分，如果其中的缝合点组经过了contour的起终点，排序会导致点点缝合错位
    如果一个点点缝合组中其中的一个缝合点组上存在相邻点间距过大，拆分它
        缺点：
            上一步的点点缝合如果存在错位，这一步会导致拆分出点点缝合组也是错位的
    筛掉太短的点点缝合组
    获取缝合边
        对每个点点缝合组中的2个缝合点组，直接取List上两端的点作为端点
        一个点点缝合组可得到一对缝合边（每个缝合边包含一对端点），这一缝合边对被缝合在一起
    对于端点离矢量化版上的板片端点很近的缝合边，将缝合边端点吸附到矢量化版的端点
    对于同一contour上相邻的缝合边，如果两者相邻端点距离近，则将这两个端点位置放到一起
    筛掉太短的缝合边对
    如果存在一对缝合边对完全相邻，将它们合并

第三版的逻辑：
    获取N个点点缝合组（Nx2个缝合点组）
        对每个contour，获取其上的所有缝合点对应的点点缝合
        对这些点点缝合，按照缝合两端的contour_id划分为N个点点缝合组（每组中的点点缝合的两端contour_id完全相同）
        对每个点点缝合组，在其中间距过大的地方进行断开
    筛掉太短的点点缝合组
    对每个缝合点组，计算其中的时针方向
    对一个缝合点组上的点，按照id进行聚类排序
        在排序时，用点与点在contour上顺+逆时针上的距离作为依据，并且不处理间距过大的，实现聚类的效果
        这样，不会出现第二版中的点点缝合错位错误
    如果一个点点缝合组中其中的一个缝合点组上存在相邻点间距过大，拆分它
    筛掉太短的点点缝合组
    修正错误的缝合点组的时针方向
        对于一个点点缝合组，计算点点缝合距离之和S1
        将其中一个缝合点组的顺序反向，计算反向的点点缝合距离之和S2
        如果S2<S1，则改掉其中一个缝合点组的时针方向
    获取缝合边
        根据以下条件，从缝合点组中提取对应缝合边的起终点：
            对于一个缝合点组，假设其对应的缝合边的起点出现在List的左半边
            缝合边的起终点应该满足条件：在长度最短的情况下，覆盖缝合点组内的所有点
        一个点点缝合组可得到一对缝合边（每个缝合边包含一对端点），这一缝合边对被缝合在一起
    对于端点离矢量化版上的板片端点很近的缝合边，将缝合边端点吸附到矢量化版的端点
    对于同一contour上相邻的缝合边，如果两者相邻端点距离近，则将这两个端点位置放到一起
    筛掉太短的缝合边对
...
def cal_neigbor_points_index_dis(start_point, end_point, all_contour_info):
    ...
    if end_point['id'] >= start_point['id']:
        index_dis=[
          contour_info['index_end'] - start_point['id'] + end_point['id'] - contour_info['index_start']+1,
...
def optimize_stitch_edge_list_byNeighbor(stitch_edge_list_paramOrder, all_contour_info, all_edge_info,
    ...
    # 获取一个list上最小值的位置
    get_min_index = lambda lst: lst.index(min(lst))
    ...
    for se_idx, stitch_edge in enumerate(stitch_edge_list_paramOrder):
        ...
        # [todo] 预留，可在这里将采样频率考虑进来
        # adjust_param = 1  # 这个用于根据采样频率调整阈值的大小
        ...
        side_dis_list = [pre_right_dis2side, cur_left_dis2side]


        target_global_param_list = []
        # todo 对缝合边端点进行优化需要加一个前提，就是优化目标位置不能跨过自身的另一个端点
        ...
        #             # if side_point_closet[0]["global_param"] == side_point_closet[1]["global_param"]:
        #             if min(cal_neigbor_points_param_dis(side_point_closet[0], side_point_closet[1], all_contour_info))< 1e-3:
        ...
        #         elif pre_right_dis2side < optimize_thresh_side_index_dis or cur_left_dis2side < optimize_thresh_side_index_dis:
        ...
        #                     if not (min(pre_right_point_dis_2side) < min(pre_left_point_dis_2side) and min(cur_left_point_dis_2side)<optimize_thresh_neighbor_index_dis):
        #                         pass
        #                     else:
        ...
        #                     pre_left_point_dis_2side = cal_neigbor_points_index_dis(pre_left_point, target_side_point, all_contour_info)    # 缝边左点 到 端点 的双向距离
        #                     cur_right_point_dis_2side = cal_neigbor_points_index_dis(cur_right_point, target_side_point, all_contour_info)  # 缝边右点 到 端点 的双向距离
        #                     if not (min(cur_left_point_dis_2side) < min(pre_left_point_dis_2side) and min(cur_right_point_dis_2side) < optimize_thresh_neighbor_index_dis):
        #                         pass
        #                     else:
        #                         target_global_param = target_side_point["global_param"]
        #                         target_global_param_list = [target_global_param, target_global_param]
        ...
        if not optimized:
            ...
            if index_dis < optimize_thresh_neighbor_index_dis and not (pre_right_dis2side<1e-3 or cur_left_dis2side<1e-3):
                if min_dis_index == 1:  # 两个边中间有缝隙
                    ...
                    #     optimized=True
                    # else: pass
                    if index_dis <= max(optimize_thresh_neighbor_index_dis/4, 2):
                        target_global_param = cur_left_point["global_param"] + param_dis / 2
                        optimized=True
                    else: pass
            # 应用结果
        ...
        # # 其中仅一个缝合边端点离板片端点特别近的情况 === V2
        # if not optimized:
        #     if index_dis < optimize_thresh_neighbor_index_dis:
        #         # 如果仅一个点离端点特别近，则将目标设为这个端点
        #         if (pre_right_dis2side < optimize_thresh_side_index_dis) ^ (cur_left_dis2side < optimize_thresh_side_index_dis):
        #             index = side_dis_list.index(min(side_dis_list))
        #             target_global_param_list = [side_point_closet[index]["global_param"], side_point_closet[index]["global_param"]]
        #             optimized = True
        #
        #         # # 如果仅一个点离端点特别近，则将目标设为这个端点
        #         # elif (pre_right_dis2side < optimize_thresh_side_index_dis) and (cur_left_dis2side < optimize_thresh_side_index_dis):
        #         #     if side_point_closet[0]["global_param"] == side_point_closet[1]["global_param"]:
        #         #         index = side_dis_list.index(min(side_dis_list))
        #         #         target_global_param_list = [side_point_closet[index]["global_param"], side_point_closet[index]["global_param"]]
        #         #         optimized = True
        #
        #         # 如果两个点都离端点特别近（或特别远），则不进行优化
        #         else:
        #             optimized=False
        #
        #     # 应用结果
        ...
        # if not optimized:
        #     if index_dis < optimize_thresh_neighbor_index_dis:
        #         # # 如果两个点都离端点不是特别近，则将目标设为两个端点的中间
        #         # if (pre_right_dis2side > 1e-3) and (cur_left_dis2side > 1e-3):
        #             # 两个边中间有缝隙
        #             if min_dis_index == 1:
        #                 if index_dis < optimize_thresh_neighbor_index_dis:
        #                     target_global_param = pre_right_point["global_param"] + param_dis / 2
    ...
    return stitch_edge_list_paramOrder
...
def pointstitch_2_edgestitch3(batch, inf_rst, stitch_mat, stitch_indices,
    ...
    :param auto_adjust                  # auto adjust parameters by delta in data_info.json
    :return:
    """

    if auto_adjust:
        raise NotImplementedError
        data_info_fp = glob(os.path.join(os.path.dirname(batch['mesh_file_path'][0]),"data_info.json"))[0]
        with open(data_info_fp,"r") as data_info:
            data_info = json.load(data_info)
            delta = data_info["delta"]
            auto_adjust_para = 0.023 / delta
        optimize_thresh_side_index_dis *= auto_adjust_para
        unstitch_thresh *= auto_adjust_para
        fliter_len *= auto_adjust_para
        optimize_thresh_neighbor_index_dis *= auto_adjust_para
    ...
    pass # FUNCTION OLD 这种缝合边的获取方法存在局限性 ============
    # all_stitch_popassints_list = []
    # stitch_points_list = []
    # unstitch_num = 0
    #
    ...
    #             # 将它们标记为缝合点
    #             lst = []
    #             if point_idx > 0 and point_idx_cor > 0:
    #                 lst.append(all_point_info[point_idx.item()])
    #                 lst.append(all_point_info[point_idx_cor.item()])
    #                 for p_l in lst:
    #                     for p in p_l:
    #                         p["is_stitch"] = True
    #
    ...
    # FUNCTION New 这种缝合边获取方法能过滤掉更多错误缝合 ============
    ...
    for contour_id in all_contour_info:
        ...
        thresh_side_dis = unstitch_thresh  # 和上一个缝合之间的间距阈值
    ...
    # 一个边上的点间距过长，则拆分边 -------------------------------------------------------------------------------------------------
    # [pl for pl in all_stitch_points_list if (pl[0][0]["contour_id"] == list(all_contour_info.keys())[10] and pl[0][1]["contour_id"] == list(all_contour_info.keys())[11])]
    ...
    # 将 离边端点特别近 且 不与其它缝边相衔接 的缝边的端点 的位置进行调整 ---------------------------------------------------------
    optimize_stitch_edge_list_byApproxEdge(stitch_edge_list, max(optimize_thresh_side_index_dis/2, 2), all_contour_info, all_edge_info)
    ...
    #     # if list(all_contour_info.keys())[idx] == contour_id:
    #     #     a=1
    #     # else:
    #     #     continue
    #
    #     # from utils import pointcloud_visualize
    #     # pointcloud_visualize(pcs[torch.concat([torch.tensor([e["end_point"]["id"] for e in edge_group]), torch.tensor([e["start_point"]["id"] for e in edge_group])])])
    #     edge_group = list(edge_group)
    #     edge_group = sorted(edge_group, key=lambda x: (x["start_point"]["global_param"] if not x["isCC"] else x["end_point"]["global_param"]))
